...
- **Repair is bounded and progress is measured, never declared.** Only a verifier `FAIL` (checks ran, some failed) can be repaired; `TIMEOUT`/`ERROR`/`INVALID` are infrastructure outcomes and end the run with a coded verdict, never an amendment. After a verifier FAIL the orchestrator records a deterministic progress fingerprint (`mas/orchestrator/progress.py`) and decides: repeat → `NO_PROGRESS`; `max_replans` spent → `BUDGET_EXHAUSTED`/`NO_PROGRESS`; else one more amendment. `max_replans` is the only repair budget (never add another). Amendments add new tasks on COMPLETED work only (validator rule 9); recorded tasks are never altered. Every non-passing run carries one `verdict_reason` code (ADR-008 §6), set only by `state_machine.fail_run/abort_run`.
