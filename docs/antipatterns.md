...
| C2 | **No / incomplete verification** (MAST 3.2) | "Looks good" | Fixed external acceptance suite, written before the run, run by deterministic code (ADR-003) | ✅ fail-closed sandboxed runner (7A) + trusted adapters (7B); 🟡 real verdicts in service mode need the verifier service (7C) |
...
| # | Anti-pattern | Why it fails | Our countermeasure | Status |
|---|---|---|---|---|
| E1 | **Cost explosion** (Anthropic: ~15× tokens; discuss.md) | Nobody counts | Usage recorded per attempt and per run; hard `max_tokens`/`max_cost_usd`; cost is a first-class evaluation axis | ✅ accounting; 🟡 real usage (step 9) |
