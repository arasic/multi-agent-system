...
- **Verifier** (deterministic, `mas/verifier/`): triggered by the orchestrator when the integration task is `COMPLETED`. Checks out the integration commit into a clean environment (ephemeral container; isolated subprocess acceptable early), mounts `acceptance/<benchmark>/` read-only, runs the suite, records a `verification` artifact + `verify.passed|failed` event with the full report.
