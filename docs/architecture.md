...
runs
  ...
  verdict_reason        text        -- ADR-008 §6 reason code for non-passing terminal runs (NULL on PASS):
                                    ...
                                    -- | CANCELLED (ADR-009: an operator ended the run; today only `mas plan`)
...
- `RUNNING → REPLANNING`: any task reaches `FAILED` (retries exhausted; `run.replanning` trigger `task_failed`, its BLOCKED dependents listed) or a worker reports `new_work_required` (kept on the task; trigger `new_work_required`, fired once per report), when a planner is configured and replans remain. Otherwise: a FAILED task → `FAILED UNRECOVERABLE_FAILURE`; new work → `task.new_work_deferred`, the run continues. Claims stop in REPLANNING; the planner is asked only once no attempt is RUNNING (**quiesce** — work in flight settles first, so the amendment builds on everything that completed and never races a live worker; attempt runtime caps bound the wait). FAILED tasks an amendment answered are marked `repair_handled` and do not re-trigger.
...
- After publishing, the worktree is removed; every attempt's branch and commits stay in the bare repo (audit). Worktrees left by dead workers are pruned when the run ends (`gc_run`, plus the service's periodic reconciliation) — never on the per-attempt path: `git worktree prune` is global, so a prune racing a sibling attempt's `git worktree add` deletes the worktree being created. `add -f` tolerates a still-registered path in the meantime.
- Every *administrative* git command on a bare repo (`branch -f`, `worktree add|remove|prune`) is serialized by `repo_admin_lock`: an advisory file lock inside the bare repo (cross-process, released by the OS if the holder is killed) plus an in-process mutex. `git worktree add` creates `worktrees/<id>/` before writing its `gitdir`/`commondir`, so a sibling command enumerating that directory dies on the half-born entry — seen at width 16. Creating a worktree **fails closed** where the filesystem cannot lock across processes (ADR-010): workers are separate processes and containers, so the in-process mutex alone would leave the race open, silently and only under load. Cleanup/GC skip instead of failing, since they run outside that window.
...
- **Conflict representation (MVP):** two `candidate` artifacts for the same output slot (same task output, or two tasks answering the same question) *are* a conflict. Resolution produces a `decision` artifact naming winner, loser(s), and rationale; losers become `superseded` or `rejected`. Disagreement stays visible in the store. No claims table (ADR-004). **As built (A7, 2026-08-17):** the runtime computes a task's *competing inputs* — same slot `<type>:<name>` from different tasks (`contracts.competing_inputs`; commits never compete by name, merges handle them) — and hands them to the agent (`ctx.competing`; the LLM brief lists them as data). The consuming task **must** publish one `decision` artifact per competing slot (`ref=decision:<slot>`, meta `slot`/`winner`/`losers`/`rationale`; the LLM `finish` tool has `decisions[]`, the stub a `decide` policy). `leases.report` validates each decision against the competing ids it handed over (an agent cannot crown an artifact it was never given; a forged or uncontested decision is an invalid decision → attempt FAILED) and applies it in the same transaction: winner `candidate → accepted`, losers `candidate → superseded_by winner`, event `artifact.decided`. A missing decision is an output-contract violation (`decision:<slot>`) — choosing silently fails the attempt. Where the competing candidates are the same file path, input assembly also leaves a git conflict; the agent applies its decision by taking the winner's version. Demo: `benchmarks/forced_disagreement/dag.json` (`mas run --dag … --stub-verifier`, then `mas artifacts <run>`); tests `tests/test_conflicts.py`.
...
### 8c. Bounded repair as built (step 13, 2026-08-17)

All three triggers are implemented end to end — verifier FAIL (13-lite), task FAILED, `new_work_required` — plus cancellation of obsolete pending work:
...
3. **Amendment protocol** (`runs.plan_run` on a REPLANNING run): the planner receives the recorded tasks with outputs, the bounded failure report (`trigger`: `verification_failed` with the check results · `task_failed` with every unhandled FAILED task's attempts and failure reasons and the BLOCKED keys · `new_work_required` with the worker's detail), the amendment hashes already tried, and remaining budgets, and must return `kind=dag` with **new tasks only** that may depend on existing COMPLETED tasks (after a verification failure typically the last integration task; after a task failure the failed task's own inputs) and end in a new integration sink; it may list `cancel: [keys]` — existing PENDING/READY tasks the amendment makes obsolete, transitioned to CANCELLED at install (rule 9: never RUNNING/COMPLETED work; an initial plan may not cancel). The validator applies rules 1–8, 10 to the amendment plus **rule 9**: no id may collide with a recorded task (COMPLETED work is never removed or altered), dependencies and `artifacts_from` may name existing tasks only if COMPLETED, an auto-appended sink gets a fresh id; the driver additionally rejects an amendment whose hash repeats an earlier one. Rejections go back as data within `max_plan_attempts`; exhaustion is a verdict (`NO_PROGRESS` when the last rejections were repeats, `INVALID_PLAN` otherwise). The newest integration task is the run's sink for verification and acceptance; the old sink's outputs stay candidates forever.
4. **Verdict reason codes** (ADR-008 §6, `runs.verdict_reason`, migrations 0007/0009, set only by `state_machine.fail_run/abort_run`): budget aborts → `BUDGET_EXHAUSTED`; invalid plans/amendments and planner output failures → `INVALID_PLAN` (only-policy rejections → `POLICY_DENIED`; unmappable acceptance criteria → `UNSUPPORTED`); task retries exhausted, no runnable work, verification failed without a planner to repair → `UNRECOVERABLE_FAILURE`; the repair decisions above → `NO_PROGRESS` / `BUDGET_EXHAUSTED`; an operator ending a run deliberately → `CANCELLED` (ADR-009; today only `mas plan`, whose run exists to produce a validated plan and is never executed — evidence tooling must treat it as no evidence at all). `mas status`/`mas run` print `reason=`.
...
**Planning without executing** (`mas plan`, ADR-009): the same driver on a run created only to plan. One `plan_run`
round — metered and charged like any other — then the validated DAG recorded in the run's `plan` artifact
(`runs.planned_dag`) is written out as a `mas run --dag` file identified by `planner.dag.plan_digest`, and the run is
ended `ABORTED` / `CANCELLED` because execution was never requested. Questions or a contract proposal end it the same
way, unexecuted, and are reported to the caller. The M3 harness uses this so configs C and D of one (N, repetition)
replay one identical plan; nothing else in the runtime treats a plan-only run as evidence.
