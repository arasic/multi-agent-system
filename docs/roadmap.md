...
- [~] 8. **End-to-end run with stub workers**: hand-written DAG → workers emit fixture commits → integration → verifier PASS. This becomes the permanent LLM-free CI test. `mas replay` works. *(e2e with stub artifacts + stub verifier is the current CI test — 80 tests, no API key; becomes complete once 6 and 7 land)*
