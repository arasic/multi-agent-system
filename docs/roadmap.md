...
- [~] 7. **Fail-closed acceptance runner** — **7A done** (576f95d + dc76304: exact-SHA checkout, hardened Docker sandbox, structured PASS/FAIL/TIMEOUT/ERROR/INVALID, bounded capture, `expected_suite_sha256` pinning, five LLM-free fixtures). **7B done** (trusted adapters `build_succeeds` / `tests_required` / `http_status` / `restart_persists`: typed schema shared by host and sandbox, trusted runner baked into the image, contract-based suites forced onto that runner with matching check ids, unmappable → INVALID at approval and verification time; `acceptance/url_shortener_contract/`; `mas contract`; fixtures for good / no-tests / broken-endpoint / volatile apps + unknown criterion). **7C pending**: concurrent run ticking (bounded executor, one connection per active tick, per-run lock, never the same run twice, no row locks during verify) and a **verifier service** so service-mode runs get real verdicts (Compose orchestrator still runs `--stub-verifier`).
...
4. ~~Step 7A: fail-closed acceptance runner~~ — done (+ review fixes). ~~Step 7B: trusted adapters~~ — done. Next: 7C concurrent ticking + verifier service. Next: trusted adapter library, then concurrent run ticking.
